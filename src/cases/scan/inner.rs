...
pub fn create_task(inputs: &[Box<[u64]>], temps: &[Box<[BlockInfo]>], outputs: &[Box<[AtomicU64]>]) -> Task {
  assert!(inputs.len() != 0);
  reset(&temps[0]);
  Task::new_dataparallel::<Data>(run, finish, Data{ inputs, temps, outputs }, ((inputs[0].len() as u64 + BLOCK_SIZE - 1) / BLOCK_SIZE) as u32)
}
